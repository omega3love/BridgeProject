...
class Reader(pygame.Rect,object):
    ...
    def update(self,ev):
        ...
        if ev.type == pygame.KEYDOWN:            
            ...
            try:
                if ev.click[4]:
                    scrollup(sum(range(1,ev.click[4]+1))//10+1)
                    return True
                elif ev.click[5]:
                    scrolldown(sum(range(1,ev.click[5]+1))//10+1)
                    return True
            except:
                if ev.button == 4:
                    scrollup(3)
...
QUIT"""
    ...
    txt = Reader(unicode(text.expandtabs(4),'utf8'),(20,20),660,15,height=360,font=os.path.join(thisrep,'MonospaceTypewriter.ttf'),bg=pygame.image.load(os.path.join(thisrep,'bg.jpg')),fgcolor=(255,255,255),hlcolor=(255,10,150,100),split=False)
    txt.show()
