...
from PygameReadWrite.form import Form
...
if charperline < len(title): charperline = len(title)
...
while 1:
    ...
    if gr:
        ...
        continue
    break
foo = text.Text(label_copy,charperline)
n_ = 0
if title: n_ += 2
if not len(foo): n_ += 1
...
foo.screen(scr,(x,y),fgcolor,font,interline=2)
char_h += 2
...
for i in fields:
    i.CURSOR = 0
    #draw.rect(scr,fgcolor,i.inflate(2,2),1)
    i.screen()
fields[0].CURSOR = 1
fields[0].screen()
index = 0
