...
class MenuSystem(list,object):
    ...
    w_arrow = pg.image.load(BytesIO(b64decode(b'iVBORw0KGgoAAAANSUhEUgAAAAkAAAAJ\
CAYAAADgkQYQAAAAAXNSR0IArs4c6QAAAAZiS0dEAAAAAAAA+UO7fwAAAAlwSFlzAAALEwAA\
CxMBAJqcGAAAAAd0SU1FB9sJFwoXK9BbsxsAAAAZdEVYdENvbW1lbnQAQ3JlYXRlZCB3aXRo\
IEdJTVBXgQ4XAAAA10lEQVQY012OMU6EABQF5xES9QT2NsTK0sKaS3AGOQE3MBa2tHAKra02\
2W5D2A0FMRJMpJBEGoj/2+xuXKd985LB3e/N7NLMAva4O3+RmT0BX5I27r6W9C5pOciSCIEf\
SebuN5Iid2/MbANUkhwglATgwAKEwDVwJenWzF6DINjJzB4ljcBJ0/4c1HX9Fv4bHNA8zxrH\
8Xu1Wu2SJHk5SpI0TdPZMAwfXddti6JY53k+HJuWZTlvmmbs+/65qqomTdPPwzmOY2jb9qEs\
y7ssyy6iKOJk3PMLcXtzAc+EJZwAAAAASUVORK5CYII=')))

    b_arrow                               = w_arrow.copy()
    b_arrow.fill(0)
    pg.surfarray.pixels_alpha(b_arrow)[:] = pg.surfarray.array_alpha(w_arrow)
    ...
    class Menu_(pg.Rect,object):
        ...
        def __init__(self,items,pos,font):
            ...
            pg.Rect.__init__(self,pos,(w+10,(self.item_h)*len(items)))
        ...
        def update(self,ev):
            ...
            if ev.type == pg.MOUSEMOTION and self.collidepoint(ev.pos):
                x,y             = ev.pos
                self.item_index = (y-self.y)//(self.item_h)
        ...
        def screen(self):
            ...
            for e,i in enumerate(self.items):
                ...
                if type(i)==Menu:
                    self.scr.blit(MenuSystem.w_arrow if e not in self.exc else MenuSystem.b_arrow,(self.right-10,b+(self.item_h-9-self.margin)//2))
                b+=self.item_h
            s = self.inflate(-2,-2)
